...
module.exports = (iploc, data) => {
  ...
  const result = {
    ...
    continent: {
      code: geodata.continent_code,
      names: {},
    },
    country: {
      iso_code: geodata.country_iso_code,
      names: {},
    },
    isp: {
      asn: isp.asn.length > 0 ? isp.asn : null,
      org: isp.org.length > 0 ? isp.org : null,
    },
    location: {},
  };

  if (iploc.accuracy_radius && iploc.accuracy_radius.length > 0) result.location.accuracy_radius = iploc.accuracy_radius;
  ...
  if (geodata.subdivision_1_iso_code.length > 0 || geodata.subdivision_2_iso_code.length > 0) {
    result.subdivisions = [];
  }

  if (geodata.subdivision_1_iso_code.length > 0) {
  ...
  if (geodata.subdivision_2_iso_code.length > 0) {
